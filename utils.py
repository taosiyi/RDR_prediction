...
def process_file_SF(file_path, window_size, step_size, compute_global,
    ...
    for i in range(0, n - window_size - 1, step_size):
        ...
        target_soc = np.float32(df['soc'].iloc[i + window_size])
...
# =========== TRANSFER LEARNING =============
def train_battery_model_TL(
        model, save_dir, train_dataset, val_dataset, optimizer,
        batch_size=32, epochs=100, device='cuda', patience=15,
        grad_clip=1.0, lambda_recon=0.1, lambda_soc=0.5
    ):
    os.makedirs(save_dir, exist_ok=True)
    loss_history = []
    ...
    print(f"Training [TL] for {epochs} epochs...")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            bs = lumped.size(0)
            train_metrics['total'] += loss.item() * bs
            train_metrics['recon'] += loss_recon.item() * bs
            train_metrics['soc'] += loss_soc.item() * bs
            train_metrics['dr5'] += loss_dr5.item() * bs
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                v_total = v_dr5 + lambda_soc * v_soc + lambda_recon * v_recon

                bs = lumped.size(0)
                val_metrics['total'] += v_total.item() * bs
                val_metrics['recon'] += v_recon.item() * bs
                val_metrics['soc'] += v_soc.item() * bs
                val_metrics['dr5'] += v_dr5.item() * bs
        ...
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            best_epoch = epoch
            torch.save(model.state_dict(), os.path.join(save_dir, f"best_TL.pth"))
        elif epoch - best_epoch >= patience:
